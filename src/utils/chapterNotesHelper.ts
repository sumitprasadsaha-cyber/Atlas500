...
import { normalizeChapterKey } from "./classNoteHelper";
...
export function groupAndSortChapterNotes(notes: ChapterNote[]): ChapterGroup[] {
  ...
  for (const note of notes) {
    ...
    const cleanTitle = getCleanChapterTitle(chInfo.chapterName) || (chInfo.chapterNo > 0 ? `Chapter ${chInfo.chapterNo}` : "General");

    if (!groupsMap.has(chInfo.normalizedChapterKey)) {
      groupsMap.set(chInfo.normalizedChapterKey, {
    ...
    const group = groupsMap.get(chInfo.normalizedChapterKey)!;
    if (
      cleanTitle &&
      (!group.chapterName || /^(?:chapter|module)\s*\d+$/i.test(group.chapterName)) &&
      !/^(?:chapter|module)\s*\d+$/i.test(cleanTitle)
    ) {
