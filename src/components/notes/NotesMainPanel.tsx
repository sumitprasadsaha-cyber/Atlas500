...
export default function NotesMainPanel({
  ...
  return (
    ...
    >
          ...
          {selectedSubject && (
            <>
              ...
              <div className="flex items-center gap-1.5 w-full sm:w-auto justify-end">
                ...
                {/* + Add Chapter/Module Button or Read-Only Indicator */}
                {!isReadOnly ? (
                  <button
                    type="button"
                  ...
                  <span 
                    className="px-2.5 py-1 bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60 rounded-lg text-xs font-bold flex items-center gap-1"
                    title={`Curriculum canonically owned by ${ownerClassName || 'owner class'}`}
                  >
      ...
      >
        {/* Read-Only Consumer Access Banner */}
        {isReadOnly && selectedSubject && (
          <div 
            className="flex items-center gap-2.5 px-3.5 py-2.5 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-xs text-amber-800 dark:text-amber-200 shadow-2xs max-w-6xl mx-auto mb-3"
            id="curriculum-readonly-banner"
          >
            <Shield className="w-4 h-4 shrink-0 text-amber-600 dark:text-amber-400" />
            <div className="flex-1 leading-relaxed">
              <span className="font-bold">Read-Only Consumer Access:</span> This curriculum is canonically owned and managed by <strong className="font-extrabold text-amber-900 dark:text-amber-100">{ownerClassName || "the owner class"}</strong>. All chapters, topics, PDFs, and practice tests are accessible in read-only mode. Switch to <strong className="font-extrabold text-amber-900 dark:text-amber-100">{ownerClassName || "the owner class"}</strong> to add or edit content.
            </div>
          </div>
        )}
        {!selectedParentName ? (
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* Right: Chapter Action Buttons (+ Upload Note, Rename, Delete) or Read-only badge */}
                    <div 
                    ...
                    >
                      {!isReadOnly ? (
                        ...
                        <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md flex items-center gap-1 border border-slate-200/60 dark:border-slate-700/60">
                          <Shield className="w-3 h-3 text-amber-500" />
                          <span>Owner: {ownerClassName || 'another class'}</span>
                        </span>
                      )}
                    </div>
                  </div>
                      ...
                      {notes.length === 0 ? (
                        <div className="py-5 px-3 text-center border border-dashed border-slate-200 dark:border-slate-800/80 rounded-xl bg-white/70 dark:bg-slate-900/50">
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            {isReadOnly
                              ? `No topic notes available for this ${itemLabel.toLowerCase()} yet.`
