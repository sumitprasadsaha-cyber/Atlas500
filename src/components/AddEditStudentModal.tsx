...
export default function AddEditStudentModal({
  ...
  useEffect(() => {
    if (studentToEdit) {
      ...
      const rawSubjs = Array.isArray(studentToEdit.enrolledSubjects) ? studentToEdit.enrolledSubjects : [];
      setEnrolledSubjects(Array.from(new Set(rawSubjs.filter(s => typeof s === "string" && s.trim()).map(s => s.trim()))));
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    if (val && !enrolledSubjects.includes(val)) {
                      setEnrolledSubjects(Array.from(new Set([...enrolledSubjects, val])));
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {enrolledSubjects.map((subj, idx) => (
                  ...
                  >
                    ...
                    <button 
                      type="button" 
                      onClick={() => setEnrolledSubjects(enrolledSubjects.filter((_, i) => i !== idx))}
