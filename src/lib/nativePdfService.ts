...
export async function fetchNoteBlobWithCache(
  ...
  try {
    ...
    console.warn("[NoteDeliveryPipeline] Cache check notice:", cacheErr);
  }

  // 2. Cancel any prior download and revoke previous Blob Object URL
  ...
  // 3. Offline check: only fail if not already found in local cache
  ...
  // 4. Resolve single canonical proxy download URL
  const targetUrl = options.canonicalUrl || getCanonicalNoteDownloadUrl(options, bucket);
  ...
  if (!response.ok || (response.status !== 200 && response.status !== 206)) {
    ...
    notesLogger.error("DOWNLOAD_ERROR", {
      storageKey: canonicalStoragePath,
      fileName,
      status: responseStatus,
      ...(errorDetails ? { extra: { details: errorDetails } } : {}),
    });
