...
async function test7_VersionConsistencyAcrossCodebase() {
  console.log("\n[Test 7] Release 7.12.9 Version Consistency Audit");

  const pkg = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), "package.json"), "utf-8"));
  assert(pkg.version === "7.12.9", `package.json version is 7.12.9 (found "${pkg.version}")`);

  const manifest = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), "public/manifest.json"), "utf-8"));
  assert(manifest.version === "7.12.9", `manifest.json version is 7.12.9 (found "${manifest.version}")`);

  const versionConstantContent = fs.readFileSync(path.resolve(process.cwd(), "src/constants/version.ts"), "utf-8");
  assert(versionConstantContent.includes('"7.12.9"'), `src/constants/version.ts exports version 7.12.9`);

  const versionApiContent = fs.readFileSync(path.resolve(process.cwd(), "api/_lib/version.ts"), "utf-8");
  assert(versionApiContent.includes('baseVersion = "7.12.9"'), `api/_lib/version.ts defaults to baseVersion 7.12.9`);
}
