...
async function runTests() {
  ...
  assert(
    APP_VERSION === "7.12.8",
    `APP_VERSION constant is "7.12.8" (actual: ${APP_VERSION})`
  );
  assert(
    pkgJson.version === "7.12.8",
    `package.json version is "7.12.8" (actual: ${pkgJson.version})`
  );
  assert(
    manifestJson.version === "7.12.8",
    `manifest.json version is "7.12.8" (actual: ${manifestJson.version})`
  );
