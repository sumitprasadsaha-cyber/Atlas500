...
async function runTests() {
  ...
  assert(
    APP_VERSION === "7.12.9",
    `APP_VERSION constant is "7.12.9" (actual: ${APP_VERSION})`
  );
  assert(
    pkgJson.version === "7.12.9",
    `package.json version is "7.12.9" (actual: ${pkgJson.version})`
  );
  assert(
    manifestJson.version === "7.12.9",
    `manifest.json version is "7.12.9" (actual: ${manifestJson.version})`
  );
