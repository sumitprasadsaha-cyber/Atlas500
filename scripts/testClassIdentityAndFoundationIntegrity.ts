...
async function runTests() {
  ...
  assert(
    APP_VERSION === "7.12.5",
    `APP_VERSION constant is "7.12.5" (actual: ${APP_VERSION})`
  );
  assert(
    pkgJson.version === "7.12.5",
    `package.json version is "7.12.5" (actual: ${pkgJson.version})`
  );
  assert(
    manifestJson.version === "7.12.5",
    `manifest.json version is "7.12.5" (actual: ${manifestJson.version})`
  );
