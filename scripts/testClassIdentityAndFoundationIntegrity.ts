...
async function runTests() {
  ...
  assert(
    APP_VERSION === "7.12.4",
    `APP_VERSION constant is "7.12.4" (actual: ${APP_VERSION})`
  );
  assert(
    pkgJson.version === "7.12.4",
    `package.json version is "7.12.4" (actual: ${pkgJson.version})`
  );
  assert(
    manifestJson.version === "7.12.4",
    `manifest.json version is "7.12.4" (actual: ${manifestJson.version})`
  );
