...
async function runTests() {
  ...
  assert(
    APP_VERSION === "7.9.10",
    `APP_VERSION constant is "7.9.10" (actual: ${APP_VERSION})`
  );
  assert(
    pkgJson.version === "7.9.10",
    `package.json version is "7.9.10" (actual: ${pkgJson.version})`
  );
  assert(
    manifestJson.version === "7.9.10",
    `manifest.json version is "7.9.10" (actual: ${manifestJson.version})`
  );
