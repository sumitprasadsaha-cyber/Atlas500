import { parseChapterTest, convertToAssessmentQuestions } from "../src/lib/testParser";
...
const parsed = parseChapterTest(sampleInput, {
  classGrade: "Class 10",
...
const converted = convertToAssessmentQuestions(parsed, {
  classGrade: "Class 10",
  subject: "Social Science",
  chapterNo: 3,
  chapterName: "Money and Credit",
...
console.log("Converted questions count:", converted.questions.length);
converted.questions.forEach((q, idx) => {
  console.log(`Q${idx + 1} (${q.displayNumber}): id=${q.id} secId=${q.sectionId} secTitle="${q.sectionTitle}" type=${q.type} marks=${q.marks} passId=${q.passageId} caseId=${q.caseId} isSubQ=${(q as any).isSubQuestion}`);
});
